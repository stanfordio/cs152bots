...
class State(Enum):
    ...
    DISMISINFO = auto()
    DISMISINFO_CATEGORIES = auto()
    MARGINALIZED_GROUPS = auto()
    SWAY_OPINIONS = auto()
    ...
    async def handle_message(self, message):
        ...
        if self.state == State.CONTENT_TYPE_IDENTIFIED:
            ...
            if (message.content == '5'):
                ...
                return ["What were the methods used to sway others\' beliefs? \nRespond with 1 for Dis/Misinformation.\nIf not Dis/Misinformation, please type your answer in 20 words or less."]
        ...
        if self.state == State.DISMISINFO:
            self.state = State.DISMISINFO_CATEGORIES
        ...
        if self.state == State.DISMISINFO_CATEGORIES:
            if (message.content == "1"):
                self.state = State.MARGINALIZED_GROUPS
                self.message['dismisinfo_category'] = "Marginalized Groups"
                return ["Who\'s being targeted? Please limit your response to 10 words."]
            elif (message.content == "2"):
                self.state = State.READY_TO_SUBMIT
                self.message['dismisinfo_category'] = "Factually Incorrect"
        
        if self.state == State.MARGINALIZED_GROUPS:
            self.message['target_group'] = message.content
            self.state = State.READY_TO_SUBMIT
